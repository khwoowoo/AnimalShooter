...
void MonsterLast::Init()
{
	...
	m_BossUISprite.LoadTextureFromFile("Resource/UI/ui_last.png");
...
void MonsterLast::Update(float deltatime)
{
	if (m_bEff)
	{
...
void MonsterLast::Destory()
{
}

bool MonsterLast::CollisionTo(IObject * obj)
{
	if (obj->Tag == "Player")
	{
		//어쩌다 보니 밀리기가 됨.
	...
	if (obj->Tag == "PlayerAtt")
	{
		...
		if (D3DXVec3Length(&vTemp) < m_fDamage)
		{
			...
			if (!m_pAtt->isDestory)
			{
				...
				cout << "내가 MonsterLast공격으로 인한 플레이어총알 삭제" << endl;
			}
			m_fHP -= m_pAtt->fPower;
			m_bDamage = true;
			return true;
	...
	if (obj->Tag == "PlayerLS")
	{
		...
		if (D3DXVec3Length(&vTemp) < m_fDamage)
		{
			if (!m_pAtt->isDestory)
			{
				m_pAtt->isDestory = true;
				SceneManager::GetInstance()->GetNowScene()->m_ObjectManager.RemoveObject(m_pAtt);
				cout << "내가 MonsterLast공격으로 인한 플레이어레이져 삭제" << endl;
			}
			m_fHP -= m_pAtt->fPower;
			m_bDamage = true;
			return true;
...
bool MonsterLast::CollisionFrom(IObject * obj)
{
	return false;
}

void MonsterLast::SetUp(LPCSTR filename, Vector3 Pos, int eMoveState, int eWeaponState, float speed)
{
	Init();
